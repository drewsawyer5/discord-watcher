...
_BARE_URL_RE = re.compile(r'(?<!\w)((?:https?://)?(?:www\.)?(?:x|twitter)\.com/\S+)', re.IGNORECASE)
...
RETRY_QUEUE_FILE = VAULT_PATH / "7 - MD-AI" / "07 - Pipeline State" / "_retry_queue.md"
...
- Always include _log.md append. The file already has a header row — append exactly one bare data row with NO extra formatting, NO table headers, NO blank lines before or after it:
  ...
  Example: | 2026-04-09 | article | My Title | https://example.com | 5 - Storage/06 - Sources/My-Title.md |
...
- For image: describe what's in the image, write a Source page to 5 - Storage/06 - Sources/ (or the appropriate wiki section per the schema), include _log.md append as usual
...
- NEVER write to vault folders 1–4 (1 - Homepage/, 2 - Projects/, 3 - Work/, 4 - Personal/). Allowed write targets: 6 - Wiki Hub/ (presentation pages), 5 - Storage/06 - Sources/ (capture Source pages), 7 - MD-AI/07 - Pipeline State/_log.md (the queue append). Routing to 1–4 is handled by /digest in a separate Claude session — not here.
"""
    return _system_prompt
...
def _process_message(msg: dict) -> bool:
    ...
    msg_id = msg["id"]
    ok = True
    full_content = _BARE_URL_RE.sub(
        lambda m: m.group(1) if m.group(1).lower().startswith("http") else f"https://{m.group(1)}",
        msg.get("content", "")
    )
